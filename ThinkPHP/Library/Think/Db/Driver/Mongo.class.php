...
class Mongo extends Driver {
    ...
    protected function multiConnect($master=false) {
        ...
        // 取得复制集服务器个数
        $configSize = sizeof($_config['hostport']) > sizeof($_config['hostname']) ? sizeof($_config['hostport']) : sizeof($_config['hostname']);

        for ($i=0; $i < $configSize; $i++) {
            $db_config[] = array(
                'username'  =>  isset($_config['username'][$i])?$_config['username'][$i]:$_config['username'][0],
                'password'  =>  isset($_config['password'][$i])?$_config['password'][$i]:$_config['password'][0],
                'hostname'  =>  isset($_config['hostname'][$i])?$_config['hostname'][$i]:$_config['hostname'][0],
                'hostport'  =>  isset($_config['hostport'][$i])?$_config['hostport'][$i]:$_config['hostport'][0],
                'database'  =>  isset($_config['database'][$i])?$_config['database'][$i]:$_config['database'][0],
                'dsn'       =>  isset($_config['dsn'][$i])?$_config['dsn'][$i]:$_config['dsn'][0],
                'charset'   =>  isset($_config['charset'][$i])?$_config['charset'][$i]:$_config['charset'][0],
            );
        }

        // 组装数据库连接字符串
        foreach ($db_config as $k => $v) {
            $hosts[] = 'mongodb://'.($v['username']?"{$v['username']}":'').($v['password']?":{$v['password']}@":'').$v['hostname'].($v['hostport']?":{$v['hostport']}":'').'/'.($v['database']?"{$v['database']}":'');
        }


        // 循环连接数据库
        foreach ($hosts as $k => $v) {
            $status = 1; // 数据库连接状态，1已连接，0未连接
            $linkNum = $k; // 重置数据库连接ID号
            if (isset($this->linkID[$linkNum])) break; // 已存在该数据库连接，直接跳出循环

            try{
                $this->linkID[$linkNum] = new \mongoClient( $v,$this->config['params']);
				
				// 循环查找主节点
				foreach($this->linkID[$linkNum]->getHosts() as $k => $v){
					// 找到主节点,跳出循环
					if($v["state"] == 1){
						break 2;
					}
                ...
                $status = 0;
                // 如果最后一个数据库配置无法连接，抛出数据库连接异常
                if ($linkNum >= $configSize) {
                    E($e->getmessage());
                }
            }
            // 数据库连接成功，跳出循环
